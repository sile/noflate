...
fn sample_input(ctx: &mut TestCaseContext) -> Vec<u8> {
    let len = noprop::sample_usize_in(ctx, 0..=MAX_INPUT);
...
fn sample_chunks(ctx: &mut TestCaseContext) -> Vec<usize> {
    ...
    (0..n).map(|_| noprop::sample_usize_in(ctx, 1..=128)).collect()
}
...
#[test]
fn noflate_decompresses_flate2_deflate() -> noprop::TestResult {
    run(|ctx| {
        let input = sample_input(ctx);
...
#[test]
fn noflate_decompresses_flate2_zlib() -> noprop::TestResult {
    run(|ctx| {
        let input = sample_input(ctx);
...
#[test]
fn noflate_decompresses_flate2_gzip() -> noprop::TestResult {
    run(|ctx| {
        let input = sample_input(ctx);
    ...
    noprop::Runner::new(SEED).run(32, |ctx| {
        ...
        for _ in 0..n_cmds {
            match sample_cmd(ctx) {
                Cmd::Feed(bytes) => {
                    encoder.feed(&bytes).expect("feed must succeed before finish");
                    expected.extend_from_slice(&bytes);
                }
                Cmd::SyncFlush => {
                    encoder.sync_flush().expect("sync_flush must succeed before finish");
                }
                Cmd::ResetHistory => {
